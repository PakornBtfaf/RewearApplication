...
class _ProductDetailPageState extends State<ProductDetailPage> {
  ...
  Future<void> _fetchLiveStatus() async {
    ...
    try {
      ...
      final res = await supabase
          .from('products')
          .select('id, status, name, price, size, condition, category, description, image_url, seller_id, seller_name')
          .eq('id', productId)
          .maybeSingle();
  ...
  Widget _buildBottomButton(
      BuildContext context, bool isSold, bool isOwnProduct) {
    // กำลังโหลดสถานะ
    if (_loadingStatus) {
      return Container(
        height: 50,
      ...
      child: ElevatedButton(
        onPressed: () {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (_) => CheckoutPage(product: _product)),
